...
const chat = async (req, res) => {
    ...
    const contents = req.body.question

    if (!titleId) {

        // creat a new conversition 
        const title = message.slice(0, 30)
        const userMessage = { type: 'q', message: message }
        ...
        // add message in old conversition 
        const userMessage = { type: 'q', message: message }
        await messageModel.create({
            conversationId: titleId,
            message: userMessage
    ...
    try {
        ...
        const AImessage = { type: 'a', message: responce.text }
        await messageModel.create({
            conversationId: titleId,
            message: AImessage
        })
...
export { chat, chatHistory, messageHistory, deleteChat }